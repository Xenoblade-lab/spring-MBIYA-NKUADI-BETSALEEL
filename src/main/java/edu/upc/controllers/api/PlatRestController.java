...
public class PlatRestController {
	...
	@PostMapping
	public ResponseEntity<?> create(@Valid @RequestBody PlatDto dto, BindingResult result) {
		Plat row = service.create(dto, result);
		return new ResponseEntity<>(row, HttpStatus.CREATED);
	}

	@PutMapping("/{id}")
	public ResponseEntity<?> update(@PathVariable("id") long id, @Valid @RequestBody PlatDto dto,
			BindingResult result) {
		Plat row = service.update(id, dto, result);
		return new ResponseEntity<>(row, HttpStatus.OK);
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<?> delete(@PathVariable("id") long id) {
		List<Plat> data = service.delete(id);
