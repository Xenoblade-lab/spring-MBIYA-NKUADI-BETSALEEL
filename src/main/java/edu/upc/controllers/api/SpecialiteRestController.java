...
public class SpecialiteRestController {
	...
	@PostMapping
	public ResponseEntity<?> create(@Valid @RequestBody SpecialiteDto dto, BindingResult result) {
		Specialite row = service.create(dto, result);
		return new ResponseEntity<>(row, HttpStatus.CREATED);
	}

	@PutMapping("/{id}")
	public ResponseEntity<?> update(@PathVariable("id") long id, @Valid @RequestBody SpecialiteDto dto,
			BindingResult result) {
		Specialite row = service.update(id, dto, result);
		return new ResponseEntity<>(row, HttpStatus.OK);
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<?> delete(@PathVariable("id") long id) {
		List<Specialite> data = service.delete(id);
