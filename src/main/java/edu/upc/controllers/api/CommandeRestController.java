...
public class CommandeRestController {
	...
	@PostMapping
	public ResponseEntity<?> create(@Valid @RequestBody CommandeDto dto, BindingResult result) {
		Commande row = service.create(dto, result);
		return new ResponseEntity<>(row, HttpStatus.CREATED);
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<?> delete(@PathVariable("id") long id) {
		List<Commande> data = service.delete(id);
